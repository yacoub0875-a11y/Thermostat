...
const heartChart = new Chart(document.getElementById('heartChart').getContext('2d'), {
    type: 'line',
    data: { labels, datasets: [{ label: 'bpm', data: heartData, borderColor: 'red', backgroundColor:'rgba(255,0,0,0.2)', fill:true, tension:0.3 }] },
    options: { responsive:true, plugins:{legend:{display:false}} }
});

const stepsChart = new Chart(document.getElementById('stepsChart').getContext('2d'), {
    type: 'line',
    data: { labels, datasets: [{ label: 'Pas', data: stepsData, borderColor: 'lime', backgroundColor:'rgba(0,255,0,0.2)', fill:true, tension:0.3 }] },
    options: { responsive:true, plugins:{legend:{display:false}} }
});
...
setInterval(() => {
    ...
    heartData.push(hr); heartData.shift(); heartChart.update();

    let st = Math.floor(1000 + Math.random()*5000);
    steps.innerHTML = st.toLocaleString();
    stepsData.push(st); stepsData.shift(); stepsChart.update();
    ...
    // AI Tips
    const tips = ["Réduire chauffage à 21°C la nuit","Activer mode éco automatique","Prévoir chauffage avant retour maison","Maintenir température constante pour économie"];
    aiTips.innerHTML = "";
    tips.sort(() => 0.5 - Math.random()).slice(0,2).forEach(tip => {
        const li = document.createElement("li"); li.textContent = tip; aiTips.appendChild(li);
    });
